...
public class QuarksShieldSyncResource {
    ...
    public List<Quark> receive() {
        List<Message> messages = sqs.receiveMessage(
                receibeRequestBuilder -> receibeRequestBuilder.maxNumberOfMessages(10).queueUrl(queueUrl)
        ).messages();

        List<Quark> quarks = messages.stream().map(Message::body).map(this::toQuark).collect(Collectors.toList());

        messages.forEach(
                message -> sqs.deleteMessage(
                        deleteRequestBuilder -> deleteRequestBuilder.receiptHandle(message.receiptHandle()).queueUrl(queueUrl)
                )
        );
        return quarks;
    }
